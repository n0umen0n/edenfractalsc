...
void eosnameswaps::sell(name account4sale, asset saleprice, name paymentaccnt,
                        string message) {
  ...
  // Only the account4sale@owner can sell (contract@eosio.code must already be
  // an owner)
  ...
  check(is_account(paymentaccnt),
        "Sell Error: The payment account does not exist.");

  // Check the payment account is not the account4sale
  check(paymentaccnt != account4sale,
        "Sell Error: The payment account cannot be the account for sale!");

  // Check the transfer is valid
  check(saleprice.symbol == network_symbol,
        (string("Sell Error: Sale price must be in ") + symbol_name +
         string(". Ex: 10.0000 ") + symbol_name + string("."))
            .c_str());
  check(saleprice.is_valid(), "Sell Error: Sale price is not valid.");
  check(saleprice >= asset(10000, network_symbol),
        (string("Sell Error: Sale price must be at least 1 ") + symbol_name +
         string(". Ex: 1.0000 ") + symbol_name + string("."))
            .c_str());

  // Check the message is not longer than 100 characters
  check(message.length() <= 100,
        "Sell Error: The message must be <= 100 characters.");

  // Invalidate any past MSIGs
  action(permission_level{account4sale, name("owner")}, name("eosio.msig"),
         name("invalidate"), std::make_tuple(account4sale.value))
      .send();
  ...
  _stats.modify(itr_stats, _self, [&](auto &s) { s.num_listed++; });

  // Send message
  send_message(
      paymentaccnt,
      string("EOSNameSwaps: Your account ") + name{account4sale}.to_string() +
          string(" has been listed for sale. Keep an eye out for bids, and "
                 "don't forget to vote for accounts you like!"));
}

// Action: Buy an account listed for sale
void eosnameswaps::buy(name from, name to, asset quantity, string memo) {

  // ----------------------------------------------
  // Auth checks
  // ----------------------------------------------

  // Important: The transfer fees actions below will trigger this function
  // without this
  if (from == _self)
    return;
  ...
  check(buy_code == "cn:" || buy_code == "sp:" || buy_code == "mk:",
        "Buy Error: Malformed buy string.");

  // Check the transfer is valid
  check(quantity.symbol == network_symbol,
        (string("Buy Error: You must pay in ") + symbol_name + string("."))
            .c_str());
  ...
  const string active_key =
      memo2.substr(name_length + 2 + KEY_LENGTH, KEY_LENGTH);

  string referrer = "";
  if (memo2.length() > name_length + 3 + 2 * KEY_LENGTH &&
      memo2.length() <= name_length + 3 + 2 * KEY_LENGTH + 12) {
  ...
  if (buy_code == "cn:") {
    ...
    buy_saleprice(account_name, from, quantity, owner_key, active_key,
                  referrer);
  } else if (buy_code == "mk:") {
...
void eosnameswaps::buy_custom(const name account_name, const name from,
                              const asset quantity, const string owner_key,
                              const string active_key) {
  ...
  check(suffix == ".e" || suffix == ".x" || suffix == ".y" || suffix == ".z",
        "Custom Error: That is not a valid suffix.");
...
void eosnameswaps::make_account(const name account_name, const name from,
                                const asset quantity,
                                const string owner_key_str,
                                const string active_key_str) {
...
authority eosnameswaps::keystring_authority(string key_str) {
  ...
  key_weight kweight{.key = {(uint8_t)key_type::k1, key_char},
                     .weight = (uint16_t)1};
...
void eosnameswaps::buy_saleprice(const name account_to_buy, const name from,
                                 const asset quantity, const string owner_key,
                                 const string active_key,
  ...
  check(itr_accounts != _accounts.end(),
        (string("Buy Error: Account ") + account_to_buy.to_string() +
         string(" is not for sale."))
            .c_str());
  ...
  if (quantity != saleprice) {
    ...
    if (quantity == itr_bids->bidprice) {
      ...
      if (bid_decision == BID_ACCEPTED) {
        ...
        check(bid_decision != BID_REJECTED,
              "Buy Error: The bid has been rejected. Bid higher.");

        // Check the bid has been decided
        check(bid_decision != BID_UNDECIDED,
              "Buy Error: The bid has not been accepted or rejected yet.");
      }

      // Check the bid is from the accepted bidder
      check(itr_bids->bidder == from, "Buy Error: Only the accepted bidder can "
                                      "purchase the account at the bid price.");
  ...
  check(saleprice == quantity,
        (string("Buy Error: You have not transferred the correct amount of ") +
         symbol_name + string(". Check the sale price."))
            .c_str());
  ...
  if (referrer.length() > 0) {
    ...
    if (itr_referrer != _referrer.end()) {
      ...
      auto referrerfee =
          asset(int(referrer_pc * contractfee.amount), network_symbol);
      contractfee.amount -= referrerfee.amount;

      // Transfer EOS from contract to referrer fees account
      action(permission_level{_self, name("active")}, name("eosio.token"),
             name("transfer"),
             std::make_tuple(_self, itr_referrer->ref_account, referrerfee,
                             string("EOSNameSwaps: Account referrer fee: ") +
  ...
  action(permission_level{_self, name("active")}, name("eosio.token"),
         name("transfer"),
         std::make_tuple(_self, feesaccount, contractfee,
                         string("EOSNameSwaps: Account contract fee: ") +
                             itr_accounts->account4sale.to_string()))
      .send();
  ...
  action(permission_level{_self, name("active")}, name("eosio.token"),
         name("transfer"),
         std::make_tuple(_self, itr_accounts->paymentaccnt, sellerfee,
                         string("EOSNameSwaps: Account seller fee: ") +
                             itr_accounts->account4sale.to_string()))
      .send();
  ...
  // Remove contract@owner permissions and replace with buyer@active account and
  // the supplied key
  account_auth(itr_accounts->account4sale, from, name("active"), name("owner"),
               active_key);

  // Remove seller@active permissions and replace with buyer@owner account and
  // the supplied key
  account_auth(itr_accounts->account4sale, from, name("owner"), name(""),
               owner_key);
  ...
  // Send message
  send_message(
      from, string("EOSNameSwaps: You have successfully bought the account ") +
                name{account_to_buy}.to_string() +
                string(". Please come again."));
}

// Action: Remove a listed account from sale
void eosnameswaps::cancel(name account4sale, string owner_key_str,
  ...
  auto itr_accounts = _accounts.find(account4sale.value);
  check(itr_accounts != _accounts.end(),
        "Cancel Error: That account name is not listed for sale");

  // Only the payment account can cancel the sale (the contract has the owner
  // key)
  check(has_auth(itr_accounts->paymentaccnt) || has_auth(_self),
        "Cancel Error: Only the payment account can cancel the sale.");
  ...
  account_auth(account4sale, itr_accounts->paymentaccnt, name("active"),
               name("owner"), active_key_str);

  // Change auth from contract@owner to submitted owner key
  account_auth(account4sale, itr_accounts->paymentaccnt, name("owner"),
               name(""), owner_key_str);
  ...
  // Send message
  send_message(itr_accounts->paymentaccnt,
               string("EOSNameSwaps: You have successfully cancelled the sale "
                      "of the account ") +
                   name{account4sale}.to_string() +
                   string(". Please come again."));
}
...
void eosnameswaps::remove(name account4sale) {
  ...
  // Only the contract account can remove the sale (the contract has the owner
  // key)
  check(has_auth(_self),
        "Cancel Error: Only the contract account can remove the sale.");
...
void eosnameswaps::update(name account4sale, asset saleprice, string message) {
  ...
  auto itr_accounts = _accounts.find(account4sale.value);
  check(itr_accounts != _accounts.end(),
        "Update Error: That account name is not listed for sale");

  // Only the payment account can update the sale price
  check(has_auth(itr_accounts->paymentaccnt),
        "Update Error: Only the payment account can update a sale.");
  ...
  // Check the transfer is valid
  check(saleprice.symbol == network_symbol,
        (string("Update Error: Sale price must be in ") + symbol_name +
         string(". Ex: 10.0000 ") + symbol_name + string("."))
            .c_str());
  check(saleprice.is_valid(), "Update Error: Sale price is not valid.");
  check(saleprice >= asset(10000, network_symbol),
        (string("Update Error: Sale price must be at least 1 ") + symbol_name +
         string(". Ex: 1.0000 ") + symbol_name + string("."))
            .c_str());
  ...
  _accounts.modify(itr_accounts, itr_accounts->paymentaccnt,
                   [&](auto &s) { s.saleprice = saleprice; });

  // Place data in extras table. Payment account pays for ram storage
  auto itr_extras = _extras.find(account4sale.value);
  _extras.modify(itr_extras, itr_accounts->paymentaccnt,
                 [&](auto &s) { s.message = message; });

  // Send message
  send_message(itr_accounts->paymentaccnt,
               string("EOSNameSwaps: You have successfully updated the sale of "
                      "the account ") +
                   name{account4sale}.to_string());
}

// Action: Increment votes
void eosnameswaps::vote(name account4sale, name voter) {
  ...
  check(has_auth(voter),
        "Vote Error: You are not who you say you are. Check permissions.");
  ...
  // Check an account with that name is listed for sale
  auto itr_extras = _extras.find(account4sale.value);
  check(itr_extras != _extras.end(),
        "Vote Error: That account name is not listed for sale.");

  // Can only vote once in a row
  check(voter != itr_extras->last_voter,
        "Vote Error: You have already voted for this account!");
...
void eosnameswaps::regref(eosio::name ref_name, eosio::name ref_account) {
  ...
  check(has_auth(_self),
        "Referrer Error: Only the contract account can register referrers.");
  ...
  check(is_account(ref_account),
        "Referrer Error: The referrer account does not exist.");
...
void eosnameswaps::regshop(name shopname, string title, string description,
                           name payment1, name payment2, name payment3) {
  ...
  check(has_auth(_self),
        "Referrer Error: Only the contract account can register shops.");
...
void eosnameswaps::proposebid(name account4sale, asset bidprice, name bidder) {
  ...
  check(
      has_auth(bidder),
      "Propose Bid Error: You are not who you say you are. Check permissions.");
  ...
  // Check an account with that name is listed for sale
  auto itr_bids = _bids.find(account4sale.value);
  check(itr_bids != _bids.end(),
        "Propose Bid Error: That account name is not listed for sale");

  // Check the transfer is valid
  check(bidprice.symbol == network_symbol,
        (string("Propose Bid Error: Bid price must be in ") + symbol_name +
         string(". Ex: 10.0000 ") + symbol_name + string("."))
            .c_str());
  check(bidprice.is_valid(), "Propose Bid Error: Bid price is not valid.");
  check(bidprice >= asset(10000, network_symbol),
        (string("Propose Bid Error: The minimum bid price is 1.0000 ") +
         symbol_name + string("."))
            .c_str());

  // Only accept new bids if they are higher
  check(bidprice > itr_bids->bidprice,
        "Propose Bid Error: You must bid higher than the last bidder.");

  // Only accept new bids if they are lower than the sale price
  auto itr_accounts = _accounts.find(account4sale.value);
  check(bidprice <= itr_accounts->saleprice,
        "Propose Bid Error: You must bid lower than the sale price.");
  ...
  // Send message
  send_message(
      itr_accounts->paymentaccnt,
      string("EOSNameSwaps: Your account ") + name{account4sale}.to_string() +
          string(" has received a bid. If you choose to accept it, the bidder "
                 "can purchase the account at the lower price. Others can "
                 "still bid higher or pay the full sale price until then."));
}

// Action: Accept or decline a bid for an account
void eosnameswaps::decidebid(name account4sale, bool accept) {
  ...
  auto itr_accounts = _accounts.find(account4sale.value);
  check(itr_accounts != _accounts.end(),
        "Decide Bid Error: That account name is not listed for sale.");

  // Only the payment account can accept bids
  check(has_auth(itr_accounts->paymentaccnt),
        "Decide Bid Error: Only the payment account can decide on bids.");
  ...
  check(itr_bids->bidprice != asset(0, network_symbol),
        "Decide Bid Error: There are no bids to accept or reject.");
  ...
  if (accept == true) {
    ...
    // Bid accepted
    _bids.modify(itr_bids, itr_accounts->paymentaccnt,
                 [&](auto &s) { s.bidaccepted = 2; });

    // Send message
    send_message(
        itr_bids->bidder,
        string("EOSNameSwaps: Your bid for ") + name{account4sale}.to_string() +
            string(" has been accepted. Account ") +
            name{itr_bids->bidder}.to_string() +
            string(" can buy it for the bid price. Be quick, as others can "
                   "still outbid you or pay the full sale price."));
  } else {

    // Bid rejected
    _bids.modify(itr_bids, itr_accounts->paymentaccnt,
                 [&](auto &s) { s.bidaccepted = 0; });

    // Send message
    send_message(itr_bids->bidder,
                 string("EOSNameSwaps: Your bid for ") +
                     name{account4sale}.to_string() +
                     string(" has been rejected. Increase your bid offer"));
  }
}
...
void eosnameswaps::null() {
  ...
  check(has_auth(_self),
        "Message Error: Only the contract can call the null action.");

  // ----------------------------------------------
}

// Message Action
void eosnameswaps::message(name receiver, string message) {
...
void eosnameswaps::screener(name account4sale, uint8_t option) {
  ...
  check(screened >= 0 && screened <= 2,
        "Admin Error: Malformed screening data.");

  // Place data in table. Contract pays for ram storage
  auto itr_extras = _extras.find(account4sale.value);
  _extras.modify(itr_extras, _self, [&](auto &s) { s.screened = screened; });
...
void eosnameswaps::send_message(name receiver, string message) {
  ...
  action(permission_level{_self, name("active")}, name("eosnameswaps"),
         name("message"), std::make_tuple(receiver, message))
      .send();
}

// Changes the owner/active permissions
void eosnameswaps::account_auth(, name change1, name change2, name perm_child,
                                name perm_parent) {
...
// Changes the owner/active permissions
void eosnameswaps::account_auth(name account4sale, name changeto,
                                name perm_child, name perm_parent,
                                string pubkey_str) {
  ...
  if (pubkey_str != "None") {
    ...
    // TEST WHETHER THIS ONE HAS TO BE DUPLICATED TO ADD ANOTHER ACC.
    //  Account to take over permission changeto@perm_child
    permission_level_weight accounts{.permission =
                                         permission_level{changeto, perm_child},
                                     .weight = (uint16_t)1};
  ...
  // Remove contract permissions and replace with changeto account.
  action(permission_level{account4sale, name("owner")}, name("eosio"),
         name("updateauth"),
         std::make_tuple(account4sale, perm_child, perm_parent,
                         contract_authority))
      .send();
} // namespace eosio
